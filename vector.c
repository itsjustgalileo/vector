...
bool vector_pop_back(Vector *vec, void **out_elem) {
    ...
    --vec->elem_count;
    memcpy(out_elem, ((char *)vec->value + (vec->elem_count * vec->elem_size)), vec->elem_size);
    return true;
}

void *vector_at(const Vector *vec, const uint32_t idx) {
    assert(vec);
...
bool vector_insert(Vector *vec, const uint32_t at, const void *src) {
...
bool vector_erase(Vector *vec, const uint32_t at) {
